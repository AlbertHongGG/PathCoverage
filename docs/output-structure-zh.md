  ...
  analysis/
    ...
    path_scatter/
      ...
      <project>/
        ...
        paths-10/
        ...
    spec_transition_compare/
      function_completion_dumbbell.png
      loc_stability_scatter.png
      cognitive_complexity_scatter.png
      cyclomatic_complexity_scatter.png
      summary.json
```
...
### 4.4 `output/analysis/spec_transition_compare/`

這一層不是 path coverage 的延伸，而是讀取 `專案基本測量數據/data.json` 之後，直接針對 15 個 project 的 SPEC-only 與 SPEC+TRANSITION 版本做摘要型報告圖。

可用指令：

```bash
uv run spec_transition_metrics_compare.py
```
...
- `function_completion_dumbbell.png`：比較 `[Func] Spec` 與 `[Func] Spec+Transition`，用來展示加入 transition 資訊後，已實作功能數與完成率通常更高。
- `loc_stability_scatter.png`：比較 `[行數] S` 與 `[行數] S+T`，每個點是一個 project，X 軸是 SPEC-only 行數、Y 軸是 SPEC+TRANSITION 行數，對角線代表兩者相等，陰影區帶代表接近對角線的區域。
- `cognitive_complexity_scatter.png`：比較 `[Cog/Fn] S` 與 `[Cog/Fn] S+T`，每個點是一個 project，X 軸是 SPEC-only 的每 function 認知複雜度，Y 軸是 SPEC+TRANSITION 的對應值。
- `cyclomatic_complexity_scatter.png`：比較 `[Cyclo/Fn] S` 與 `[Cyclo/Fn] S+T`，每個點是一個 project，X 軸是 SPEC-only 的每 function 循環複雜度，Y 軸是 SPEC+TRANSITION 的對應值。

`summary.json` 會額外保存：

- 每個 project 的正規化數值
- 各指標的 delta 與 ratio change
- 四張圖各自的排序方式與摘要統計
- 圖上 `1` 到 `15` 的 display index 對應到哪一個 project
- 被跳過的資料列，例如 `ID` 與 `專案名稱` 為空的彙總列
